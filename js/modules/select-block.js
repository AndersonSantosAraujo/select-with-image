export default class SelectBlock {
  ...
  onOutsideMenu(event) {
    if (
      !(
        ...
        this.menu.classList.contains(this.classActive)
      )
    ) {
      this.menu.classList.toggle(this.classActive);
      this.onEvents(0);
      this.selectItens.forEach((item) =>
        item.removeEventListener("click", this.onClickItem),
      );
    }
  }
  ...
  onStart(index) {
    ...
    this.displayItem.attributes.id.value =
      this.selectItens[index].getAttribute("id");
  }
